...
import useFetchTeachingAssignments from '../hooks/useFetchAllTeachingAssignments';
...
const TeachingAssignmentList = () => {
    ...
    const { data, page, pageSize, totalPages, loading, setPage, setPageSize, refetchData } = useFetchTeachingAssignments();
    ...
    const handleDelete = (id: string | null) => {
        if (id) {
            ...
            if (selectedIds.length > 0) {
                deleteAssignment(selectedIds, refetchData);
                setCheckedItems({});
            }
        }
    };
    ...
    const handleItemChange = (index: number, event: React.ChangeEvent<HTMLInputElement>) => {
        console.log(`Item ID: ${index}, Checked: ${event.target.checked}`);
    ...
    const selectedIds = Object.entries(checkedItems)
        ...
        .map(([id]) => Number(id));
console.log('Received teaching assignments:', data);

    return (
      <>
        <TeachingAssignmentActions selectedIds={selectedIds} onDeleteMany={() => handleDelete(null)} />
        <section className="rounded-lg bg-background-white shadow-[4px_4px_25px_4px_rgba(154,201,245,0.25)] sm:p-5 antialiased mb-10 ">
          <div className="flex flex-col md:flex-row items-stretch md:items-center md:space-x-3 space-y-3 md:space-y-0 justify-between mx-4 py-4 dark:border-gray-700">
            <TitleComponent text="Danh sách phân công giảng dạy" size={22} weight="extrabold" style={{ fontFamily: 'var(--font-Mulish)' }} />
          </div>

          <div className="mx-4">{alert && <AlertwithIcon message={alert.message} type={alert.type} icon={alert.icon} />}</div>
        ...
        {isEditModalOpen && (
          <TeachingAssignmentFormEdit
            ...
            item={selectedItem} // Truyền dữ liệu đã chọn vào form
