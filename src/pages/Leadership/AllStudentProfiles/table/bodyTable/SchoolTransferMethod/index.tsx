...
const SchoolTransferMethod: React.FC = () => {
    ...
    <div className="flex justify-center items-center max-h-screen p-10">
      <div className="bg-white rounded-2xl w-full max-w-[884px] h-auto shadow-lg flex flex-col">
        <form className="w-full pt-3 px-6 md:px-[60px] pb-10">
          <h2 className="text-black-text text-center text-2xl font-bold mb-5">Cập nhật chuyển trường</h2>
          {/*  */}
          ...
          <div className="flex flex-col md:flex-row items-start justify-start mb-4">
            <label className="md:w-3/12 w-full text-black-text font-bold text-base mb-2 md:mb-0">Chuyển đến trường:</label>
            <input
              type="text"
              className={`w-full md:w-9/12 p-2 rounded-lg cursor-pointer transition-all ${
                isSchoolFocused ? 'border border-gray-400 bg-white' : 'border bg-gray-100'
              }`}
              onFocus={() => setIsSchoolFocused(true)}
              onBlur={(e) => setIsSchoolFocused(e.target.value !== '')}
              placeholder="Nhập tên trường"
            />
          </div>
          {/*  */}
          <div className="flex flex-col md:flex-row items-start justify-start mb-4">
            <label className="md:w-3/12 w-full text-black-text font-bold text-base mb-2 md:mb-0">Địa chỉ trường:</label>
            <input
              type="text"
              className={`w-full md:w-9/12 p-2 rounded-lg cursor-pointer transition-all ${
                isAddressFocused ? 'border border-gray-400 bg-white' : 'border bg-gray-100'
              }`}
              onFocus={() => setIsAddressFocused(true)}
              onBlur={(e) => setIsAddressFocused(e.target.value !== '')}
              placeholder="Địa chỉ trường"
            />
          </div>
          {/*  */}
          <div className="flex flex-col md:flex-row items-start justify-between mb-4">
            <label className="md:w-3/12 w-full text-black-text font-bold text-base mb-2 md:mb-0 self-start">Lí do chuyển trường:</label>
            <div className="relative w-full md:w-9/12">
              <textarea
...
export default SchoolTransferMethod;
