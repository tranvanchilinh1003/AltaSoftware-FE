        ...
        <div className="relative w-full border rounded-lg overflow-hidden">
          <table className="table-auto min-w-full border-collapse overflow-hidden rounded-t-lg">
            ...
            <tbody className="max-h-48 overflow-y-auto">
              {data.filter(item => item.status === 'Đã hoàn thành').map((item, index) => (
                <tr key={index} className={`border-b ${index % 2 === 1 ? 'bg-gray-100' : ''}`}>
                  ...
                  <td className="py-3 px-4 text-left text-green-500 italic">{item.status}</td>
                  <td className="py-3 px-4 text-left text-green-500 italic">{item.action}</td>
